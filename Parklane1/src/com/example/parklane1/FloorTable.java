...
public class FloorTable {
	...
	public FloorTable(Context context) {
	...
	public long insertData(String vrn, String ip, int pslot,int floorNo) {
		SQLiteDatabase db = helper.getWritableDatabase();
		ContentValues cv = new ContentValues();
		cv.put(MGbPAHelper.VRN, vrn);
		cv.put(MGbPAHelper.IP_Address, ip);
		cv.put(MGbPAHelper.PkSlot, pslot);
		cv.put(MGbPAHelper.Floor_No, floorNo);
	...
	static class MGbPAHelper extends SQLiteOpenHelper {
		...
		private static final String CREATE_TABLE = "CREATE TABLE " + TABLE_NAME
				...
				+ " VARCHAR(255)  NOT NULL," + PkSlot + " INT(10) ,"
				+ Floor_No + " INT(10),PRIMARY KEY("
				+ VRN + "," + IP_Address + "));";
